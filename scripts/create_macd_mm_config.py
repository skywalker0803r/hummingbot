...
class CreateMACDMMConfig(ScriptStrategyBase):
    ...
    # No __init__ method. The parent class will handle everything.

    def on_tick(self):
        # When this method is executed, Hummingbot should have already prompted the user
        # and populated self.config with the answers.
        if not self.config:
            self.notify("Error: Configuration not found. Please start the script without a --conf file.")
            self.stop()
            return
