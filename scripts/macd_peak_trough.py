...
class MACDPeakTroughStrategy(ScriptStrategyBase):  
    ...
    def on_tick(self):
        ...
        if len(self.macd_values) == 3:
            ...
            if is_positive_peak:
                if active_position and active_position.position_side == PositionSide.LONG:
                    # Close long position, then wait for the next tick to open a short one.
                    self.execute_trade(TradeType.SELL, "Closing long position", abs(active_position.amount), PositionAction.CLOSE)
                    return  # Exit for this tick to ensure close is processed first
            ...
            if is_negative_peak:
                if active_position and active_position.position_side == PositionSide.SHORT:
                    # Close short position, then wait for the next tick to open a long one.
                    self.execute_trade(TradeType.BUY, "Closing short position", abs(active_position.amount), PositionAction.CLOSE)
                    return  # Exit for this tick to ensure close is processed first
                if not active_position:
                    # Open long position
    ...
    def get_active_position(self):
        """Get the active position for the current trading pair."""
        connector = self.connectors[self.config.exchange]
    ...
    def execute_trade(self, trade_type: TradeType, reason: str, amount: Decimal, position_action: PositionAction):
        """Execute a market order with a specific position action."""
        connector = self.connectors[self.config.exchange]
    ...
    async def on_stop(self):
        """Close all open positions when the strategy stops."""
    ...
    def format_status(self) -> str:
        """Display strategy status, including balances, orders, position, and MACD info."""
