...
if TYPE_CHECKING:
    from hummingbot.connector.derivative.topone_perpetual.topone_perpetual_derivative import (
        ToponePerpetualDerivative,
    )


class TopOnePerpetualAPIOrderBookDataSource(PerpetualAPIOrderBookDataSource):
    ...
    def __init__(
            ...
            connector: 'ToponePerpetualDerivative',
    ...
    async def _request_complete_funding_info(self, trading_pair: str):
        ...
        data = await rest_assistant.execute_request(
            url=web_utils.public_rest_url(path_url=CONSTANTS.FUNDING_INFO_URL, domain=self._domain),
    ...
    async def _request_complete_contract_details(self, trading_pair: str):
        ...
        rest_assistant = await self._api_factory.get_rest_assistant()
        data = await rest_assistant.execute_request(
            url=web_utils.public_rest_url(path_url=CONSTANTS.EXCHANGE_INFO_URL, domain=self._domain),
            params={"symbol": ex_trading_pair},
            method=RESTMethod.GET,
